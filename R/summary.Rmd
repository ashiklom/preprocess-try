...
pftcols <- "clm45"
traits_pfts <- readRDS(here::here('processed/traits/traits_pfts.rds'))
all_pfts <- readRDS(here::here('processed/species/all_pfts.rds'))
species_phylo <- src_sqlite(here::here('try.sqlite')) %>% 
    tbl('species_phylo') %>% 
    collect(n = Inf)
...
```{r datasummary}
cpdat <- readRDS(here::here('processed/traits/traits_analysis.rds'))
sumlong <- cpdat %>% 
